    ...
    for i in range(1, len(calevents)):
        ...
        for j in xrange(300, int(td.total_seconds()), 3600):
            ...
            if (stime.hour > 21) or (stime.hour < 9):  # No alarms after 9pm or before 9am
                continue
            alarm_events.append({'summary': 'Alarm!', 
                                 'start': {'dateTime': get_date_string(stime)}, 
                                 'end': {'dateTime': get_date_string(stime + datetime.timedelta(seconds=5))}})
...
def get_next_alarm(services):
    ...
    for i in range(0, len(calevents)):
        ...
        if ts_start >= datetime.now(tz = pytz.timezone('US/Eastern')):
            return ts_start
...
class AlarmWindow(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.pack()
        self.button = tk.Button(self, text="Stand up and shake a leg!", font=(
            None, 24), command=self.on_press, width=300, height=300)
        self.button.pack()
