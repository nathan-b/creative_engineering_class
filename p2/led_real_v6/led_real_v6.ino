...
uint32_t shift_dynamic_color(uint32_t c_in, rgb_color tilt = NONE, int8_t inc = increment)
{
  ...
  if (tilt == RED) {
    ...
    mutate(g, inc);
    decrement_color(b, inc);
  } else if (tilt == GREEN) {
    mutate(r, inc);
    increment_color(g, inc);
    decrement_color(b, inc);
  } else if (tilt == BLUE) {
    decrement_color(r, inc);
    mutate(g, inc);
...
void build_dynamic_palette(CRGBPalette16& pal)
{
  ...
  for (int i = 0; i < 16; ++i) {
    if (i == 0) {
      ...
      cb[i] = shift_dynamic_color(cb[1], RED, 0x40);
    } else if (i == 6) {
      cb[i] = shift_dynamic_color(cb[2], GREEN, 0x40);
    } else if (i == 8 || i == 12) {
      cb[i] = get_dynamic_color();
    } else {
...
void build_dynamic_palette(uint32_t* cb)
{
  for (int i = 0; i < 16; ++i) {
    if (i == 0) {
      cb[i] = rgb_to_int(get_dynamic_value(), 0, 0);
...
void setup() {
  ...
  build_dynamic_palette(gTargetPalette);
